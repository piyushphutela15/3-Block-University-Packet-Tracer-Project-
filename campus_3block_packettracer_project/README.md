...
**How to use**
1. Open Cisco Packet Tracer (recommended version: 8.x — include the actual version you are using in your GitHub README).
2. Add devices matching the topology: 1 Core L3 switch, 3 Distribution switches, several Access switches, Router (Edge), Wireless LAN Controller (or simulated WLC), APs, Servers, PCs.
3. Use the CLI config files in `configs/` as templates: copy/paste into device CLI (enable mode) then save running-config to startup-config.
4. Configure the Packet Tracer Server DHCP/DNS using the `servers/dhcpd.conf` and `servers/named.conf` examples as guidance if you replicate on a real server.
5. Follow `lab_steps.md` to wire devices, configure VLANs, trunking, SVIs, EtherChannel, STP, DHCP, and WLC mappings.
